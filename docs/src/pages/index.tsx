...
export default function Home(): JSX.Element {
	useLayoutEffect(() => {
		...
		console.log(styles.card, cards, gridWrapper);
		...
		function updateBackgrounds() {
			cards.forEach((card) => {
				...
				div.id = id;
				div.style.width = `${card.offsetWidth}px`;
				div.style.height = `${card.offsetHeight}px`;
				div.style.top = `${card.offsetTop}px`;
				div.style.left = `${card.offsetLeft}px`;
				div.classList.add(styles.cardBackground);

				if (!existing) gridWrapper.appendChild(div);
			});
		}
		...
		if (globalThis.ResizeObserver) {
			observer = new ResizeObserver(updateBackgrounds);
	...
	return (
		<Layout wrapperClassName="main" description="The fastest react-native list renderer">
			<div className="bg-white dark:bg-black">
				<div id="grid-wrapper" className="min-h-[100vh] flex flex-col justify-center items-center px-8 py-20">
					<div
						style={{
							backgroundImage: `linear-gradient(45deg, #ff00ad 20%, #f834ff 30%, #7d4eff 40%, #1a7fff 50%, #00c3ff 60%, #00cfb7 70%, #00d177 90%)`,
						}}
					...
					>
						...
						<div id={styles.performance} className={styles.card}>
							...
							<div className={styles.big}>
								<svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
									<defs>
										<linearGradient id="pink-gradient" x2="1" y2="0.5">
